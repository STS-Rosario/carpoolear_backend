...
class UsersManagerTest extends TestCase
{
    ...
    public function test_validator_create_fails_when_emails_notifications_is_not_boolean(): void
    {
        $validator = $this->manager()->validator([
            'name' => 'Regular User',
            'email' => 'regular-bool-'.uniqid('', true).'@example.com',
            'password' => 'password12',
            'password_confirmation' => 'password12',
    ...
    public function test_validator_create_allows_missing_password_fields(): void
    {
        $validator = $this->manager()->validator([
            'name' => 'Regular User',
            'email' => 'regular-nopass-'.uniqid('', true).'@example.com',
            'emails_notifications' => true,
        ], null, false, false, false);
    ...
    public function test_validator_create_fails_when_name_exceeds_max_length(): void
    {
        $validator = $this->manager()->validator([
            'name' => str_repeat('n', 256),
            'email' => 'regular-longname-'.uniqid('', true).'@example.com',
            'password' => 'password12',
            'password_confirmation' => 'password12',
            'emails_notifications' => true,
        ], null, false, false, false);
    ...
    public function test_update_hides_trips_when_user_is_banned(): void
    {
        ...
        $editableService->shouldReceive('filterForUser')
            ->once()
            ->with(Mockery::type('array'), false, $user, null)
    ...
    public function test_update_sends_flagged_property_alert_and_still_updates_allowed_fields(): void
    {
        ...
        $editableService->shouldReceive('filterForUser')
            ->once()
            ->with($requestData, false, $user, null)
    ...
    public function test_admin_update_does_not_apply_banned_dni_rejection(): void
    {
        ...
        $editableService->shouldReceive('filterForUser')
            ->once()
            ->with($requestData, true, $user, null)
