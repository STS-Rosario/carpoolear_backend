...
- **`FileRepository.php` (Pest session, 100% score)** — prior report operators included `Line 23: UnwrapRtrim`, `Line 25: ConcatRemoveRight` / `UnwrapTrim`, `Line 39: EqualToIdentical`, `Line 50` / `66` `TrueToFalse` on `makeDirectory`, `Line 54–56` / `72–74` `EmptyStringToNotEmpty` / `UnwrapStrReplace` / `Concat*`, Imagick `readImageBlob` / `thumbnailImage` / `File::put`, GD `imagecreatetruecolor` / `imagecopyresized` / `imagejpeg`, and `\Log::error`. Example **UNCOVERED** IDs before the fix (GD path never executed under Imagick): `aa7fedc7f4a5a735`, `bd1dc157a94610d2`, `8f9e0f7ab46d227b` (plus other line-97–98 integer / `RemoveFunctionCall` IDs in the same Pest run).
  - Cause: **Path:** rtrim/trim/concat mutants could distort the testing upload root. **`nomalize`:** trailing-slash branch needed an explicit contract vs “append slash”. **`createFromFile`:** generated filenames needed a format assertion beyond “ends with `.txt`”. **`createFromData`:** when Imagick was installed, tests skipped GD so `imagecopyresized`/`imagejpeg` mutants were **UNCOVERED**; the `$name.'.'.$extension` concat on line 70 was likewise weak. **`catch (Exception)`** in `STS\Repository` did not catch PHP 8 `TypeError` from `imagesx(false)` and did not resolve to `\Exception`, so the `Log::error` path rarely ran.
  - Fix: **`protected usesImagickForThumbnails()`** (tests use **`FileRepositoryUsingGdThumbnails`** to force the GD pipeline); **`catch (\Throwable $e)`** before `\Log::error`. New/adjusted tests: `test_resolve_upload_folder_does_not_insert_double_directory_separator_after_temp`, `test_resolve_upload_folder_trims_slashes_around_folder_argument_before_joining`, `test_nomalize_trailing_slash_detection_uses_strict_slash_character`, regex on `createFromFile` output, GD subclass tests with **`getimagesize` 400×400** and generated-name regex, **`test_create_from_data_imagick_writes_400_square_when_imagick_available`**, **`Log::spy`** for Imagick bad blob and GD invalid payload.
...
- **`UserRepository.php` `migrateUsers` / `invokeMigrateUserMerge`, `friendList`, `getNotifications` default, `storeResetToken`, `addFriend('')`, `unansweredConversationOrRequestsByTrip` trip scope** — prior Pest **UNCOVERED** / weak: **`62d1d06b5dc92040`**, **`39edcc2e94f7c244`**, **`ee13ba1008c0ddd0`** (`migrateUsers` guard + merge), **`a1b26c945f8c2cbb`** (`friendList` had no `return`), **`2171a2bbde783a22`** (`migrateUser` delegate after extract); user report also listed **`Line 178: FalseToTrue`**, **`Line 153: RemoveArrayItem`**, **`Line 131` / `133–134`**, **`Line 198: RemoveMethodCall`** clusters.
  - Cause: **`User::migrateUser()`** is not defined on **`STS\Models\User`**, so a real two-user **`migrateUsers`** call fatals before mutation could assert the inner call. **`friendList`** only invoked **`$user->friends()`** and returned nothing. Omitted **`$unread`** on **`getNotifications`** was not compared to explicit **`false`**. **`password_resets`** insert was not checked for **`created_at`**. Empty-string **`origin`** and **`whereHas('trip', fn … user_id)`** on pending counts lacked focused assertions.
  - Fix: **`protected invokeMigrateUserMerge(User $userKeep, User $userToDelete)`** (tests: **`UserRepositoryMergeSpy`** + **`test_migrate_users_no_op_when_*`** + **`test_migrate_users_invokes_merge_when_both_users_exist`**; **`test_invoke_migrate_user_merge_forwards_to_migrate_user_on_keep_user`** uses reflection + **`Mockery::mock(User::class)`** so the default body’s **`migrateUser`** call is exercised). **`friendList` returns `$user->friends()->get()`** with **`test_friend_list_returns_accepted_friends_collection`**. **`test_get_notifications_omitted_unread_argument_matches_explicit_false`**, **`test_add_friend_persists_empty_string_origin`**, strengthened **`test_password_reset_token_round_trip`** (`created_at`), **`test_unanswered_conversation_or_requests_counts_only_when_trip_belongs_to_user`**.
...
- **Apr 2026 `RUN` survivors — omitted `carpoolear` keys, literal defaults, `<= 0`, POS `null`, submit OR-chain, validator loop, `storeIdentityImage` concat, 201 JSON** (user-report operators e.g. `Line 27:FalseToTrue`, `Line 30:FalseToTrue`, `Line 33:DecrementInteger`/`IncrementInteger`, `Line 43:FalseToTrue`, `Line 44:RemoveEarlyReturn`, `Line 88/91/144/147/150:FalseToTrue`, `Line 94–95` / `Line 157–158` integer/`IncrementInteger` on `<= 0`, `Line 108/170:FalseToTrue` / `RemoveArrayItem` on create payloads, `Line 153:EmptyStringToNotEmpty`, `Line 206:RemoveArrayItem`, `Line 209/212:FalseToTrue`, `Line 229:BooleanOrToBooleanAnd`, `Line 234:ForeachEmptyIterable` / `RemoveArrayItem`, `Line 236:TrueToFalse` / `RemoveNot` / `Line 237:CoalesceRemoveLeft`, `Line 241:Concat*`, `Line 255–256:RemoveArrayItem`).
  - **Cause:** Feature tests set flags explicitly, so Infection could flip `config('…', false)` / `config('…', 0)` defaults, nudge integer literals, weaken `<= 0`, drop `paid => false`, treat POS `null` like a configured id, strip `errors.request_id`, flip `||` to `&&` for the three-file gate, empty the `foreach` over uploads, change `($result['valid'] ?? true)`, or break path construction / success JSON—without failing assertions.
  - **Fix:** `carpoolearConfigWithout()` rebuilds `config('carpoolear')` minus named keys so defaults apply; new tests drive **omitted-key** branches for `cost()`, `status()` (with a persisted row), `createPreference`, `createQrOrder`, and `submit()` plus `test_qr_order_returns_service_unavailable_when_pos_external_id_is_null`, `test_preference_succeeds_when_cost_is_one_cent` / `test_qr_order_succeeds_when_cost_is_one_cent` (boundary on `<= 0`), `test_preference_persists_new_unpaid_row_with_paid_false` / `test_qr_order_persists_new_unpaid_row_with_paid_false`, `test_submit_without_request_id_returns_unprocessable` extended with `errors.request_id`, missing front/back image messages, `test_submit_accepts_validator_payload_without_valid_key_treating_as_success` (pins `?? true`), `test_submit_back_image_validation_failure_returns_field_errors`, `test_submit_stores_jpeg_under_identity_validations_prefix_when_converter_returns_bytes`, and `assertExactJson` on the `201` success body. `test_qr_order_returns_service_unavailable_when_manual_validation_disabled_before_qr_gate` pins the HTTP status literal on the `createQrOrder()` manual gate (`Line 148:DecrementInteger`/`IncrementInteger` → `502`/`504` mutants; IDs `c44f1bc642687ad3`, `b592021f7e919966`). `test_qr_order_rolls_back_new_row_when_mercado_pago_throws_before_response` pins `$validationRequest->delete()` in the QR `catch` (`Line 179:RemoveMethodCall`; ID `480e452bc085c461`).
...
- **`index()` default `$pageNumber` integer literal** (`index()` ~35 `DecrementInteger` on default `1` → `0`).
  - Cause: `make_pagination()` (`app/Helpers/Queries.php`) treats falsy `$pageNumber` as “unset” and coerces it back to `1`, so mutating the controller default to `0` still produced `meta.pagination.current_page` = `1` on `GET api/conversations` without a `page` query.
  - Fix: `test_index_forwards_default_page_one_to_conversation_logic_when_page_query_omitted` binds a `ConversationsManager` mock that expects `getUserConversations($user, 1, 20)` exactly once when `page` is omitted; a default of `0` fails the argument expectation. Mutant ID: *(re-run Infection for this path to record the hash)*.
...
- **Apr 2026 Pest scoped run (`TripController.php` + `TripApiTest` only) — Cordova `search()` meta literals (112–119), `trackSearch` defaults (148/152), log concat (157), `errorsContainCode` (259–267), Cordova `show()` stub (90–91)** (user report: `DecrementInteger`/`IncrementInteger`/`RemoveArrayItem` on 113–119; `FalseToTrue` on 148; `DecrementInteger`/`IncrementInteger` on 152; `ConcatSwitchSides` on 157; `RemoveEarlyReturn`/`RemoveArrayCast` on 260/262/265; `RemoveEarlyReturn`/`RemoveArrayItem` on 90–91).
  - Cause: `GET api/trips` under Cordova only asserted `total` + one `data` field; `trackSearch` was not argument-pinned; the tracking `Log::error` assertion allowed swapped concat substrings; `errorsContainCode` was not hit from this suite with `stdClass` payloads or `getErrors() === null`; Cordova `show()` was covered in `TripControllerIntegrationTest` but not in a `TripApiTest`-only mutation scope, leaving lines 90–91 uncovered (`UNCOVERED` IDs `bede2872654e5f92`, `875c9233aeae77c2` before the fix).
  - Fix: `TripApiTest` adds `withOldCordovaUserAgent()` + `test_search_returns_exact_legacy_meta_pagination_for_old_cordova_clients` (exact `meta.pagination` + `links`); `test_search_tracking_passes_web_platform_zero_and_passenger_false_when_flags_omitted`; `Log::shouldHaveReceived('error')->with('Error tracking trip search: tracking failed')`; `test_create_returns_default_message_when_create_fails_and_errors_are_null`; reflection helpers `test_errors_contain_code_casts_dynamic_stdclass_errors_to_array` and `test_errors_contain_code_returns_false_when_error_key_is_explicitly_null`; `test_show_returns_legacy_placeholder_for_old_cordova_without_hitting_trips_logic` (`TripsManager::show` `never()` + placeholder JSON).
...
- **`handle()` logging + mail** (`~44–88`; report ~61253–61555, e.g. `1852deb5736b67a4` `FalseToTrue` on `log_emails`, `5dea70580896df65` / `b842307513ce3f0d` `RemoveMethodCall` on `Log::info` / `Mail::send`, `57236eb0f8acc0b6` / `fd65cf8322b5fd73` `IfNegated` on the `email_logs` branch, `3a7da4ccd46ce8c5` / `394d9637b800a7c1` on `array_merge` / `substr` token masking, `e47e26da3d6ffe79` on success `Log::info`). User-report line:operator survivors included **`L46`/`L120`** `FalseToTrue`, **`L62`** `UnwrapSubstr` / int mutants on `substr`, **`L77–L79`** / **`L92–L97`** `RemoveArrayItem`, **`L105–L106`** `UnwrapArrayMerge` on `PASSWORD_RESET_EMAIL_FAILED`.
  ...
  - **Fix:** `test_handle_sends_reset_password_mailable` (`Mail::fake()`, `Mail::assertSent`). `test_handle_when_log_emails_key_is_absent_does_not_use_email_logs_channel` (`Config::set('carpoolear', …)` without `log_emails`, `Log::channel` `never`). `test_handle_logs_to_email_logs_when_enabled` — exact `substr($token,0,10).'...'` (length **13**), `PASSWORD_RESET_EMAIL_SUCCESS` includes `timestamp`. `test_handle_when_mail_fails_and_email_logging_merges_stack_trace_on_channel` — `stack_trace` + `error_code` on channel `error`. `test_handle_rethrows_after_logging_when_mail_fails` — `error_code`, `attempt`, `timestamp` on catch `errorData`.

- **`handle()` catch + `failed()`** (`~90–137`; report ~61568+ tail, e.g. `RemoveMethodCall` on `Log::error`, channel `critical`, `RemoveArrayItem` on merged failure payloads). User-report: **`L123–L127`** `RemoveArrayItem` on `failed()` `$failureData` / critical merge.
  - **Cause:** Permanent-failure path did not require `attempts` / `timestamp` on both base log and `email_logs` `critical` when the `log_emails` key was absent vs present.
  - **Fix:** `test_failed_when_log_emails_key_is_absent_does_not_use_email_logs_channel`. `test_failed_logs_permanent_failure_and_optional_email_channel` — `attempts`, `timestamp`, `stack_trace` on `PASSWORD_RESET_EMAIL_PERMANENTLY_FAILED` payloads.
...
- **`handle()` logging + `DeleteAccountRequestNotification`** (`~37–73`; report ~62015–62185, e.g. `3673e288fb7e2f34` `FalseToTrue` on `log_emails`, `6772304b906e7571` / `24b2e43e22820204` `RemoveMethodCall` on `Log::info` / `Mail::send`, `70c7460f8acccb61` / `72706fac5c64878f` `IfNegated` on `email_logs`, `d319341e339f64e0` / `b08d800737e931b7` on `array_merge` / channel `info`, `e5f437421ef30893` / `e33f84f1f5b56116` on success logging).
  ...
  - **Fix:** `test_handle_sends_delete_account_request_notification` uses `Mail::fake()` and `Mail::assertSent(DeleteAccountRequestNotification::class, …)`. `test_handle_when_log_emails_key_is_absent_does_not_touch_email_logs_channel`. `test_handle_logs_to_email_logs_when_enabled` asserts ordered `Log::info` plus two `email_logs` `info` calls (`DELETE_ACCOUNT_REQUEST_EMAIL_SENDING` with merged `admin_url`, then `DELETE_ACCOUNT_REQUEST_EMAIL_SUCCESS`). `test_handle_when_mail_fails_with_email_logging_merges_stack_trace_on_channel_error`.

- **`handle()` catch + `failed()`** (`~75–121`; report ~62197–62257+, e.g. `97ed30b28316b5bb` through `3a7eb30e949576ac` on `errorData`, `b6d5b2d9c69dae05` on `Log::error`, channel `error`/`critical` mutants). User-report: **`L104`** `FalseToTrue` on `failed()` `config(..., false)`.
  - **Cause:** `failed()` with missing `log_emails` key was not asserted to skip `email_logs`; `critical` merge did not require `attempts` alongside `stack_trace`.
  - **Fix:** `test_handle_rethrows_after_logging_when_mail_fails`. `test_failed_when_log_emails_key_is_absent_does_not_use_email_logs_channel`. `test_failed_logs_permanent_failure_and_optional_email_channel` tightened `Mockery::on` for `attempts`, `timestamp`, `stack_trace` on `DELETE_ACCOUNT_REQUEST_EMAIL_PERMANENTLY_FAILED`.
...
- **Empty resolved MP id, string cast, and log coalesce** (`success()` payment guard + assignment + `Log::info` context; Infection `Line 30:EmptyStringToNotEmpty`, `Line 31:RemoveStringCast`, `Line 37:CoalesceRemoveLeft` — line numbers drift slightly with formatting).
  - Cause: `EmptyStringToNotEmpty` weakens the `!== ''` guard so a resolved empty `$paymentId` can persist an empty `payment_id`. `RemoveStringCast` removes `(string)` so an integer `payment_id` on the request can diverge from normalized string storage. `CoalesceRemoveLeft` drops the left operand of `$paymentId ?? $validationRequest->payment_id`; that is observationally the same as the real `??` when `$paymentId` is **null** (common when HTTP normalizes `payment_id=`), but wrong when `$paymentId` is a real **empty string** (`'' ?? 'prior'` must stay `''`, not `'prior'`).
  - Fix: controller uses `Illuminate\Support\Facades\Log::info` so `Log::spy()` matches production. `test_success_when_payment_and_collection_resolve_to_empty_string_does_not_persist_empty_payment_id` calls `success()` with a `Request` query bag that sets **literal** `''` for `payment_id` and `collection_id`, then asserts `payment_id` remains `null`. `test_success_logs_empty_string_payment_context_when_resolved_identifier_is_empty_even_if_row_has_prior_payment_id` seeds a prior `payment_id`, uses the same empty-string query shape, and asserts the success log’s `payment_id` context is `''`. `test_success_normalizes_integer_payment_identifier_to_string_on_model` sets `payment_id` to an integer on the query bag and asserts the row stores `'55055'` with a string raw attribute.
...
- **Follow-up (Pest scoped, 90 mutants @ 100%)** — **`Line 156: EqualToIdentical`**, **`Line 46–50`**, **`Line 62`**
  - **Cause:** **`endsWith()`** used **`$length == 0`** (no observable difference from **`===`** for **`strlen`** integers) and loose **`substr(...) === $needle`** without normalizing types; **`descarga()`** read **`$_SERVER['HTTP_USER_AGENT']`** and used **`header()`/`die()`** (not observable under PHPUnit); **`hashPassword()`** used **`echo`/`die`** with no response when **`p`** was absent (**`IfNegated`** / **`RemoveFunctionCall`** gaps).
  - **Mutant ID:** **`Line 156:EqualToIdentical`** (historical); post-refactor **`Line 44:IfNegated`**, **`IdenticalToNotIdentical`**, **`Line 154–161`** cluster for **`endsWith`**; **`descarga`** / **`hashPassword`** lines shift to **`Line 42–63`** in the **90-mutant** run.
  - **Fix:** **`endsWith`** now normalizes with **`(string)`**, early-returns on **`$needleStr === ''`**, and compares **`substr($haystackStr, -$length) === $needleStr`**; **`descarga(Request $request)`** uses **`preg_match(...) === 1`** and **`redirect()->away(...)`**; **`hashPassword`** returns **`response(...)`** / **`response()->noContent()`**. Tests: **`test_descarga_redirects_to_app_store_for_ios_user_agent`**, **`test_descarga_redirects_to_play_store_for_non_ios_user_agent`**, **`test_hash_password_returns_plain_text_bcrypt_when_query_param_present`**, **`test_hash_password_returns_no_content_without_query_param`**, plus extended **`endsWith`** cases (including numeric suffix).
...
- **Config logging, QR conjuncts, JWT/retoken/logout/reset branches** (user-report operators on `AuthController.php` such as `Line 34:FalseToTrue`, `Line 61–68`/`Line 73` log/foreach, `Line 76–79` boolean/`RemoveNot`, `Line 92`, `Line 115`/`Line 119` `EmptyStringToNotEmpty`, `Line 148` `RemoveArrayItem`, `Line 157` `IfNegated`, `Line 181–193`, `Line 216–217`, `Line 236`/`Line 239`/`Line 244`, `Line 110` `JWTException`, `Line 162` retoken banned, `Line 197` logout `Log::error` concat).
  - Cause: `_getConfig()` logging and guest warning, exclude→flatten loops, manual-QR `&&`/`empty` gate, login `_getConfig()` default vs cordova, retoken TTL refresh / `app_version` payload / banned JSON, logout `invalidate` catch logging, reset `strpos` remaps, and `JWTAuth::attempt` failure envelope lacked focused assertions; `\Log` did not always align with `Log::spy()`.
  - Fix: `AuthController` uses `Illuminate\Support\Facades\Log` for all log calls. `AuthControllerApiTest` adds `test_login_config_banner_uses_default_non_cordova_branch_from_get_config_helper`, `test_get_config_logs_environment_check_and_guest_warning`, `test_get_config_merges_flat_carpoolear_keys_after_exclude_foreach`, `test_get_config_identity_validation_manual_qr_enabled_matches_conjunctive_gate`, `test_login_with_banned_user_returns_user_banned_message`, `test_login_with_inactive_user_returns_user_not_active_message`, `test_retoken_with_app_version_passes_session_payload_to_device_manager`, `test_retoken_after_short_ttl_expiry_refreshes_token_and_returns_config`, `test_reset_password_maps_rate_limit_exception_to_user_message`, `test_reset_password_maps_wait_minutes_exception_to_same_message`, `test_login_returns_500_when_jwt_attempt_throws_jwt_exception`, `test_retoken_with_banned_user_returns_forbidden_banned_payload` (`withoutMiddleware(UserLoggin::class)`), and `test_logout_logs_error_when_jwt_invalidate_throws` (`JWTAuth::swap` + direct `logout()`).

- **Apr 2026 `RUN` follow-up** (user-report operators e.g. `Line 34:FalseToTrue`, `Line 61–68`/`Line 73` log context `RemoveArrayItem`, `Line 92:RemoveMethodCall`, `Line 115`/`Line 119` `EmptyStringToNotEmpty`, `Line 148:RemoveArrayItem`, `Line 157:IfNegated`, `Line 181–196`, `Line 236`/`Line 239`/`Line 244` `strpos` / `BooleanOrToBooleanAnd` / `RemoveMethodCall` / `Concat*`).
  - **Cause:** `_getConfig()` guest logging and `raw_env` keys were only weakly asserted; login Cordova default could survive if the wrong branch still returned a URL string; `reset()` rate/cooldown `strpos` branches needed independent substrings; `wait` without `minutes` must not use the cooldown remap; HTTP logout did not assert the successful `Log::info` after `invalidate`; `DeviceManager::updateBySession` needed the literal `session_id` token match.
  - **Fix:** `test_get_config_logs_environment_check_and_guest_warning` now requires `raw_env` to contain `MODULE_USER_REQUEST_LIMITED_ENABLED` and `MODULE_USER_REQUEST_LIMITED_HOURS_RANGE`; `test_login_config_banner_uses_default_non_cordova_branch_from_get_config_helper` refutes the Cordova-only URL; `test_reset_password_maps_exception_with_450_code_even_without_rate_keyword`, `test_reset_password_maps_exception_with_rate_keyword_even_without_450_code`, and `test_reset_password_does_not_map_wait_only_messages_without_minutes` pin the `||` vs `&&` behavior on `reset()` catch remaps; `test_logout_with_valid_session_logs_successful_invalidation` asserts `Log::info('JWT token invalidated successfully')`; `test_retoken_with_app_version_passes_session_payload_to_device_manager` asserts `$payload['session_id'] === $token`. Re-run Infection for updated hashes.
...
- **Apr 2026 `RUN` follow-up — verification success log, POST signature path, invalid `object`, payload processing logs** (user-report survivors such as `Line 59:RemoveMethodCall`, `Line 81–82` / `Line 87` / `Line 116` / `Line 123:RemoveMethodCall`, `Line 139:BooleanOrToBooleanAnd`, `Line 140:RemoveArrayItem`/`RemoveMethodCall`, `Line 156`/`168–169`/`195`/`217:CoalesceRemoveLeft`, `Line 146`/`158`/`197`/`219:ForeachEmptyIterable`, `Line 171–174`/`179`/`182`/`185`/`198–200`/`220–222:RemoveMethodCall`/`RemoveArrayItem`).
  - **Cause:** HTTP assertions (`200`/`401`/`403`, JSON bodies) were enough for routing and guards, but **dropping `Log::info` / `Log::warning` calls or log context keys** left the suite green; **`object` guard** `||` vs `&&` and **`??` / `foreach`** branches in `processWebhookPayload` → `processChange` → `handleMessages` / `handleMessageStatus` were not tied to **observable log lines** (Meta still gets `200` even when handlers no-op).
  - **Fix:** `WhatsAppWebhookTest` now asserts: `WhatsApp webhook verification successful` after successful GET verification; both `No WhatsApp webhook signature found` and `WhatsApp webhook signature verification failed` on unsigned POST with secret configured; `WhatsApp app secret not configured` + `WhatsApp webhook event notification` (with `payload` key) when secret is unset but a non-empty signature is sent; `WhatsApp webhook signature verification failed` on bad HMAC; `WhatsApp webhook event notification` on valid signed empty entry; `Invalid webhook payload object` with `payload` shape for wrong/missing `object`; `test_post_processes_messages_message_status_and_unknown_change_fields` uses `Log::spy()` to require `Processing WhatsApp webhook change` (per `field` + `business_account_id` + `value`), `Received WhatsApp message`, `WhatsApp message status update`, and `Unhandled webhook field`; `test_post_processes_entry_when_changes_key_is_missing` pins the `entry['changes'] ?? []` path without errors. `Log::warning` matchers use variadic `...$args` because Laravel may pass a single string without a context array. Re-run Infection for fresh hashes after edits.
...
- **Mutant IDs (user report):** `L16–L19:RemoveArrayItem`, `L24:RemoveMethodCall`, `L32–L45`, `L51–L87`, `L93–L123`, `L129–L161` clusters (`RemoveArrayItem`, `RemoveMethodCall`, `IfNegated`, `RemoveEarlyReturn`, `ForeachEmptyIterable`, `CoalesceRemoveLeft`, `FalseToTrue`, etc.).
  - **Cause:** User-facing support routes needed explicit contracts for **auth**, **`middleware('logged')`**, **`show` eager loads + nested attachments**, **create/reply** validation (including **`attachments`** omitted vs present, disallowed MIMEs), **`reply`/`close` missing-ticket 404** (same JSON as `show` but those branches were not hit over HTTP), **closed-ticket reply** (`Resuelto` and `Cerrado`), **`close` without `message_markdown`** (no extra reply row), **priority** derived per type (ignoring spoofed `priority` in the body), and **persisted audit fields** (`created_by`, `last_reply_at`, reply **`is_admin`**, attachment rows, `Storage` for uploads). Pest also reported **`TYPE_DEFAULT_PRIORITY` `RemoveArrayItem` mutants as UNCOVERED** (`217d0fd37a6542e0`, `47fae25ae7d0ee01`, `1f24ccde2a7b8cd0`, `a5e2d14b0a3c2906`) because **PHPUnit line coverage does not attribute execution to `private const` entries**, so mutants survived as “uncovered” even though HTTP tests asserted priorities. **`reply`/`close` not-found mutants** (`a60789333197be77`, `43f57bb19b623b44`, `087dbcaed1957fa1`, `98c2f6d22db8e23f` on `L101`; `99979f519ca2a998`, `db85bacf07bb4d7d`, `1f9231ed2be455c9`, `776b055f08fafda9` on `L134`) were uncovered until **`POST …/replies`** and **`POST …/close`** were called for a non-existent id.
  - **Fix:** `tests/Feature/Http/SupportTicketApiTest.php` — `test_constructor_registers_logged_middleware` (instantiates the controller with a mocked `SupportTicketService` and asserts a **`logged`** middleware entry); `test_support_ticket_routes_require_authentication` (unauthenticated **`json`** matrix → **401** + `Unauthorized.`); `test_show_returns_replies_with_nested_attachments_when_present`; `test_create_succeeds_when_attachments_key_is_omitted`; **`test_create_rejects_non_image_attachment_mime`** uses **`postJson`** so validation returns **422** JSON (not a **302** session redirect); `test_create_persists_created_by_and_last_reply_at`; `test_reply_persists_user_attachment_and_advances_ticket_status` (`Storage::fake`, DB attachment count, status **`Esperando respuesta`**); `test_reply_returns_422_when_ticket_is_closed` / resolved; **`test_reply_returns_not_found_for_missing_ticket`** and **`test_close_returns_not_found_for_missing_ticket`** (`404` + **`assertExactJson(['error' => 'Ticket not found'])`**); `test_close_without_message_does_not_add_second_reply`; `test_priority_is_assigned_by_type_and_not_by_user_input` (all four **`in:`** types). **Controller:** replaced the **`private const`** map with **`private static function typeDefaultPriorities(): array`** so the default-priority table lives on **executable lines** and Pest can kill **`RemoveArrayItem` / `AlwaysReturnEmptyArray`** on that map at **100%** for this file + test pair.

## PhoneVerificationController (`app/Http/Controllers/Api/v1/PhoneVerificationController.php`)

- **Mutant IDs (user RUN report, line:operator):** `L17:RemoveArrayItem` (four entries on `only([...])`), `L17:RemoveMethodCall` (dropping `middleware('logged')->only(...)`); `L27:RemoveMethodCall`, `L28–L30:RemoveArrayItem` (first **`Log::info('Phone verification send request', [...])`** context); `L38–L39:RemoveArrayItem` (error log context keys beside **`errors`**); `L50:AlwaysReturnNull`, `L51–L53:RemoveArrayItem` (success **`response()->json([...])`** for **`send`**); `L70:AlwaysReturnNull`, `L71–L74:RemoveArrayItem` (**`verify`** success envelope); `L91:AlwaysReturnNull`, `L92–L93:RemoveArrayItem` (**`resend`** success envelope). **`L35` / `L65` / `L86` / `L105`** clusters were already killed by strict **`=== null`** checks and **`status`** passthrough. After this hardening, **`composer test:mutate`** for this file reports **35 tested, 100%**; Pest prints stable **`xxh3` IDs** mainly for **escaped/uncovered** mutants, so re-run after a deliberate regression if you need fresh hashes for CI logs.
  - **Cause (constructor `L17`):** Nothing asserted **`logged`** + **`only(['send','verify','resend','status'])`**; removing a method name from **`only`** or stripping **`middleware`** could survive.
  - **Fix:** **`test_constructor_registers_logged_middleware_only_for_phone_actions`** mirrors **`AuthControllerApiTest`**: **`getMiddleware()`** + exact **`options['only']`**.
  - **Cause (send logging `L27–L30`, error `L38–L39`):** Success path only asserted the second **`Log::info`** (“send successful”); the initial “send request” log and **`Log::error`** payload keys **`user_id` / `phone`** were not required, so **`RemoveMethodCall`** / **`RemoveArrayItem`** on those lines survived.
  - **Fix:** **`test_send_returns_json_when_manager_returns_payload`** now requires **`Log::info`** for **`Phone verification send request`** with **`user_id`**, **`phone`**, and **`ip`**, and a matching “successful” info line; **`test_send_throws_exception_with_errors_and_logs_when_manager_returns_null`** asserts the same request log plus **`Log::error`** including **`user_id`**, **`phone`**, and **`errors`**.
  - **Cause (send JSON `L50–L53`):** Partial assertions could miss **`AlwaysReturnNull`** on **`response()->json(...)`** or stripping **`message` / `phone` / `expires_in_minutes`**.
  - **Fix:** unchanged **`assertSame`** full payload on **`send`** success (kept strict).
  - **Cause (`verify` / `resend` success `L70–L74`, `L91–L93`):** Only **`null`** outcomes were tested; mutants could return **`null`** from **`response()->json`** or drop keys on the happy path.
  - **Fix:** **`test_verify_returns_json_when_manager_returns_payload`** and **`test_resend_returns_json_when_manager_returns_payload`** assert full **200** JSON bodies against a mocked **`PhoneVerificationManager`** (still one mock per test, no broad stubs).

## FriendsController (`app/Http/Controllers/Api/v1/FriendsController.php`)

- **Mutant IDs (user report, line:operator):** `L22:RemoveMethodCall` (**`middleware('logged')`**); `L31–L33`, `L45–L47`, `L59–L61`, `L73–L75` **`IfNegated`** on **`$friend` / `$ret`** guards; `L88:IfNegated` on **`isset($data['page_size'])`** (paginator vs collection); `L100:AlwaysReturnNull` on **`pedings`** return.
  - **Cause:** Only **`pedings` / `index`** had incidental coverage from other suites; **`middleware('logged')`** and the **`request` / `accept` / `delete` / `reject`** happy paths were not executed under **`composer test:mutate`** for this file, so Pest marked most **`IfNegated`** mutants **UNCOVERED** (e.g. **`07e0b2da1d56dd22`**, **`4521a373c517acb0`**, … **`95b722e2c3241221`** for **`pedings`**).
  - **Fix:** **`tests/Unit/Http/FriendsControllerTest.php`** constructs the controller with **`Mockery`** **`FriendsManager` + `UsersManager`**, **`actingAs`** a real user, and drives **`request` / `accept` / `delete` / `reject`** success (**`200`** + **`OK`**), **`request`** failure (**`ExceptionWithErrors`** when **`find`** is **`null`**), **`index`** with and without **`page_size`** (assert **`meta`** present vs absent), and **`pedings`** (**`data`** envelope). **`test_constructor_registers_logged_middleware`** pins **`RemoveMethodCall`** on **`middleware('logged')`**. Pair reaches **100%** (11 mutations tested).

## DebugController (`app/Http/Controllers/Api/v1/DebugController.php`)

- **Mutant IDs (user report, line:operator):** `L18:IfNegated` on **`$request->has('log')`**; `L19:RemoveMethodCall`, `L19:ConcatRemoveLeft` / **`ConcatRemoveRight`** / **`ConcatSwitchSides`** on **`'ERROR IN APP: '.$request->get('log')`**.
  - **Cause:** **`tests/Unit/Coverage/UncoveredInfrastructureTest`** only invoked **`log()`** without asserting **`Log`** output, so mutants could skip logging or corrupt the concatenated message.
  - **Fix:** **`tests/Unit/Http/DebugControllerTest.php`** uses **`Log::spy()`**: no payload ⇒ **`Log` must not receive `info`**; with **`log`** ⇒ exact message **`ERROR IN APP: …`**. Debug cases removed from **`UncoveredInfrastructureTest`** to avoid duplication.

## MercadoPagoOAuthController + MercadoPagoOAuthService

- **Mutant IDs (user report):** `L21–L121` clusters — **`RemoveArrayItem` / `RemoveMethodCall`** on entry **`Log::info`** and branch **`Log::warning`/`Log::error`/`Log::info`** contexts; **`IfNegated` / `RemoveNot` / `BooleanOrToBooleanAnd`** on **`$error`**, **`!$code || !$state`**, cache/user/token/name/DNI guards; **`RemoveEarlyReturn`**, **`RemoveIntegerCast`** on **`(int) $cached['user_id']`**, **`CoalesceRemoveLeft`** on **`code_verifier`**; concat mutants on **`mp_oauth_state:`**; catch **`L121`** payload; success / rejection rows and redirects.
  - **Cause:** Redirect-only assertions did not pin **structured logging** or edge branches (**cache without `user_id`**, **`users/me`** without **`identification`**, **`users/me` HTTP failure** after a good token, **numeric-string `user_id`**, **empty local `name`** name-mismatch, **DNI log** context keys, **`MercadoPagoRejectedValidation::create`** attributes).
  - **Fix:** **`tests/Feature/Http/MercadoPagoOAuthCallbackTest.php`** — extended existing flows with **`Log::spy()`** + **`shouldHaveReceived`** on message/context pairs; added **`test_redirects_to_error_when_cached_payload_lacks_user_id`**, **`test_redirects_to_error_when_users_me_fails_after_token`**, **`test_redirects_to_error_when_users_me_has_no_identification`**, **`test_redirects_name_mismatch_when_local_name_is_empty`**, **`test_resolves_user_when_cached_user_id_is_numeric_string`**, and stricter **`name_mismatch` / `dni_mismatch` / `success`** log assertions (including **`user_dni_normalized` / `mp_dni_normalized`**). **`MercadoPagoOAuthService`** non-array JSON coercion unchanged. Callback + test file: **89 mutations, 100%**.

## Admin `UserController` (`app/Http/Controllers/Api/Admin/UserController.php`)

- **Mutant IDs (user report):** `index` **`L43–L67`** (**`per_page` clamp**, **`name` trim/null**, **`sort` whitelist**, **`direction`**, multi-field **`LIKE`**); **`accountDeleteList` / `accountDeleteUpdate`** response keys; **`clearIdentityValidation`** **`L128–L136`**; **`delete` / `anonymize` / `banAndAnonymize`** **`L148–L257`** (**`AdminActionLog`**, **`BannedUser`**, note/DNI normalization, **`details`** JSON).
  - **Cause:** Earlier tests covered a subset of **`index`** and **`clearIdentityValidation`** / sparse **`ban-and-anonymize`**; mutants could still change **`per_page` floor/ceiling**, default **30**, **`last_connection`** ordering, **`nro_doc` / `mobile_phone`** search arms, invalid **`direction`** vs explicit **`desc`**, list/update JSON envelopes, **`anonymize`** vs ratings guard, digit-only **`nro_doc`** on ban, blank **`note`**, and **`admin_action_logs.details`** without failing CI.
  - **Fix:** **`tests/Feature/Http/AdminUserControllerIntegrationTest.php`** — **`test_index_default_per_page_is_thirty`**, **`test_index_per_page_zero_clamps_to_one`**, **`test_index_sort_last_connection_orders_newest_first_when_desc`** (scoped **`name`** filter), **`test_index_name_search_finds_user_by_nro_doc_fragment`**, **`test_index_name_search_finds_user_by_mobile_phone_fragment`**, **`test_index_invalid_direction_defaults_to_desc_for_name_sort`**, **`test_account_delete_list_returns_data_envelope`**, **`test_account_delete_update_response_includes_user_email`**, **`test_clear_identity_validation_returns_stable_data_keys`**, **`test_banned_users_list_returns_paginated_payload`**, **`test_anonymize_succeeds_when_user_has_no_ratings`** + **`test_anonymize_returns_unprocessable_when_user_has_negative_received_rating`**, **`test_ban_and_anonymize_stores_digits_only_nro_doc_when_present`**, **`test_ban_and_anonymize_with_blank_note_and_doc_only_stores_note_null`**, **`test_ban_and_anonymize_logs_admin_action_with_note_detail`**. **`clearIdentityValidation`** / original **`fresh()->only([...])`** fix retained. **152 mutations, 100%** for this controller + integration test path.

## Admin `SupportTicketController` (`app/Http/Controllers/Api/Admin/SupportTicketController.php`) — follow-up

- **Mutant IDs (user report):** `reply` ~`L36–L37`, `L49`, `L61–L62` (`RemoveArrayItem` on validation keys, `RemoveMethodCall` on notification wiring); **`L36`** **`RemoveArrayItem`** on **`attachments`** / **`max:3`**; **`L49`** / **`L154`** **`RemoveArrayItem`** on **`SupportTicketReply::create`** keys (**`message_markdown`**, **`created_by`**).
  - **Cause:** `attachments.*` mime validation was only covered for **happy** image uploads; **`NotificationServices::send`** was mocked only for the **throwing** path, so removing **`setAttribute('ticket'| 'from')`** or widening validation could still return **200** in some runs. **`max:3`** and **`created_by`** on persisted replies were not always asserted.
  - **Fix:** **`tests/Feature/Http/AdminSupportTicketControllerIntegrationTest.php`** — `test_reply_rejects_attachment_with_disallowed_mime` (**PDF** → **422**, multipart **`post`** with **`Accept: application/json`** so validation returns JSON instead of a **302** redirect); `test_reply_invokes_notification_services_with_ticket_from_and_ticket_owner` expects **`send`** twice (database + push channels) with a **`SupportTicketReplyNotification`** carrying **`ticket`**, **`from`**, and the ticket **owner** as the notifiable; **`test_reply_rejects_more_than_three_attachments`**; **`created_by`** asserted on **`test_reply_with_text_only_returns_fresh_ticket_under_data_and_advances_status`**, **`test_resolve_with_message_creates_admin_reply`**, **`test_close_with_message_creates_admin_reply_and_closed_fields`**.

## Admin `CampaignRewardController` (`app/Http/Controllers/Api/Admin/CampaignRewardController.php`)

- **Mutant IDs (user report):** `index` / `show` ~`L15–L17`, `L43–L45` (`RemoveArrayItem` on `where('status','paid')`, `RemoveMethodCall` on `loadCount`, wrong-campaign guard); **`update`** **`L56–L58`** **`RemoveArrayItem`** on **`description` / `donation_amount_cents` / `quantity_available`** rules.
  - **Cause:** Only the **public** purchase flow was covered under a similarly named integration test; admin **`withCount` / `loadCount`** paid-only constraints and **404** for mismatched `campaign_id` were not asserted over HTTP. **`update`** was only exercised with **`title`/`is_active`**, so stripping **`min:1`** or **`string`** rules could survive.
  - **Fix:** **`tests/Feature/Http/AdminCampaignRewardControllerIntegrationTest.php`** already covered **`index`** paid-only **`withCount`** and **`show`** **404** across campaigns; added **`test_show_load_count_excludes_pending_donations`** so **`loadCount(['donations' => fn ($q) => $q->where('status','paid')])`** cannot regress to counting **`pending`** rows on **`show`**; **`test_update_rejects_zero_donation_amount_cents`**, **`test_update_rejects_non_string_description_payload`**.

## Admin `MercadoPagoRejectedValidationController` (`app/Http/Controllers/Api/Admin/MercadoPagoRejectedValidationController.php`)

- **Mutant IDs (user report, line:operator):** `index` **`L22–L30`** (`RemoveArrayItem`, `TernaryNegated`, **`L26`** `RemoveBooleanCast` / `FalseToTrue`); **`show`** **`L41–L63`** (relation selects + payload keys + datetime ternaries); **`review`** **`L73–L131`** (validation, **`L80`** missing-user branch, **`L85`** status ternary, **`L86`** note coalesce, approve vs else identity writes, **`L108–L109`** `save`/`load`, response map **`L111–L128`**); **`approve`** **`L139`** (`merge` + delegate).
  - **Cause:** Escaped mutants dropped keys from the **`map()`** / **`show`** / **`review`** JSON arrays, flipped user null-ternaries, removed **`(bool)`** on **`identity_validated`** (truthy non-boolean would pass JSON **`assertFalse`**), inverted **`review_status`** assignment, removed **`?? ''`** on **`review_note`**, skipped **`save()`** / **`load()`**, or broke **`approve()`** forwarding—without HTTP assertions on exact shapes, orphan **`user_id`**, reject/pending rules, **`404`** when **`user` is null**, and legacy **`POST …/approve`**.
  - **Fix:** **`tests/Feature/Http/AdminMercadoPagoRejectedValidationControllerIntegrationTest.php`** — **`test_index_returns_data_newest_first_with_expected_shape`** (**`data`** envelope, first row keys, **`assertFalse((bool) user_identity_validated)`** for a real **`false`** user); **`test_index_maps_identity_validated_true_when_user_is_validated`** (pins **`(bool)`** + true branch); **`test_index_maps_null_user_fields_when_user_id_has_no_matching_row`** (orphan row: **`user_name`/`user_nro_doc` null**, **`user_identity_validated` false**); **`test_show_returns_single_row_payload_and_review_approve_updates_user`** + **`test_show_returns_full_audit_fields_after_approval`** (**`mp_payload`**, null vs set audit fields, **`user_identity_validated`** before/after); **`test_review_reject_clears_identity_and_returns_rejected_payload`**, **`test_review_pending_sets_status_and_requires_note`**, **`test_review_returns_not_found_when_user_relation_is_missing`**; **`test_post_legacy_approve_matches_review_approve`** (**`L139`** `merge` + **`review`** delegation).

## Admin `CampaignDonationController` (`app/Http/Controllers/Api/Admin/CampaignDonationController.php`)

- **Mutant IDs (user report, line:operator):** **`store`** **`L30–L37`** (`RemoveArrayItem` on validation keys); **`L41`** int mutants; **`index`** / **`show`** / **`update`** / **`destroy`** (`RemoveArrayItem`, `RemoveMethodCall` on **`with`/`load`/`create`/`update`/`delete`**); nested **`apiResource`** binding order required **`Campaign`** before **`CampaignDonation`** on **`show`/`update`/`destroy`**.
  - **Cause:** No admin feature test hit **`api/admin/campaigns/{campaign}/donations`** CRUD with **`assertDatabaseHas`/`Missing`**, embedded **`user`**, **`campaign`** on **`show`**, or **`min:1`** / **`in:…`** validation—so mutants could strip validation rules, relation calls, or persistence without failing; wrong controller signatures caused **`TypeError`** instead of exercising the real handlers.
  - **Fix:** **`tests/Feature/Http/AdminCampaignDonationControllerIntegrationTest.php`** covers **`index`** (**`with('user')`** shape), **`store`** (**`201`** + DB row), **`show`** (**`user` + `campaign`**), **`test_show_returns_not_found_when_donation_belongs_to_another_campaign`** (**`assertDonationBelongsToCampaign`**), **`update`**, **`destroy`** (**`204`** + row removed). **`CampaignDonationController`**: **`show(Campaign $campaign, CampaignDonation $donation)`**, **`update(Request, Campaign, CampaignDonation)`**, **`destroy(Campaign, CampaignDonation)`**, plus **`assertDonationBelongsToCampaign`** (**`404`** when **`campaign_id`** does not match the route **`Campaign`**).

## Admin `ManualIdentityValidationController` (`app/Http/Controllers/Api/Admin/ManualIdentityValidationController.php`)

- **Mutant IDs (user report, line:operator):** **`index`** **`L22–L41`** (`RemoveArrayItem`, `TernaryNegated` on map); **`L41`** **`RemoveArrayItem`** on **`['data' => $items]`**; **`show`** **`L51–L72`** (`UnwrapRtrim`, `ConcatRemove*`, ternary URL fields); **`image`** **`L81–L104`** (`IfNegated`, stream headers); **`review`** **`L112–L151`**; **`purge`** **`L161–L170`**.
  - **Cause:** No admin HTTP coverage asserted the **`data`** envelope, full **`map()`** keys, **`rtrim(config('app.url'))`**-based image URLs, **`Storage`** stream vs missing-file **`404`**, paid gate on **`review`**, or **`purge`** clearing paths and files.
  - **Fix:** **`tests/Feature/Http/AdminManualIdentityValidationControllerIntegrationTest.php`** — **`test_index_returns_only_data_key_and_full_row_shape`**; **`test_show_builds_image_urls_from_rtrimmed_app_url_without_double_slash`**; **`test_image_invalid_type_returns_json_not_found_with_exact_payload`** (direct **`image($id, 'invalid-type')`** because the route **`where('type', 'front|back|selfie')`** prevents reaching **`in_array`** over HTTP); **`test_image_returns_not_found_when_column_empty_or_file_missing`** + **`test_image_streams_existing_file_with_inline_disposition`**; **`test_review_rejects_unpaid_request_and_approves_paid_updates_user`**; **`test_review_reject_clears_identity_on_user_row_in_database`** (reject branch **`save()`** + **`identity_validated = false`**); **`test_purge_deletes_stored_files_and_clears_paths`**.

## Admin `CampaignController` (`app/Http/Controllers/Api/Admin/CampaignController.php`)

- **Mutant IDs (user report, line:operator):** **`index`** **`L21`** (`CoalesceRemoveLeft`, `DecrementInteger`/`IncrementInteger` on **`total_donated`**); **`store`** **`L33–L50`** (validation keys, **`has('slug')`**, **`Str::slug`**); **`show`** **`L58–L63`** (`RemoveMethodCall` on relation constraints, **`TrueToFalse`** on reward filter, **`?? 0`**); **`update`** **`L73–L87`**; **`destroy`** **`L97–L98`**.
  - **Cause:** Admin CRUD was not exercised over HTTP with assertions on **`latest()`** ordering, **`total_donated`** from paid donations only, auto- vs explicit **`slug`**, nested **`milestones` / `donations` / `rewards`** filters, title-driven slug refresh, and **`204`** delete.
  - **Fix:** **`tests/Feature/Http/AdminCampaignControllerIntegrationTest.php`** — index newest-first + **`total_donated`**; **`store`** slug generation and validation failure; **`show`** paid-only donations and active-only rewards; **`update`** slug regeneration; **`destroy`**.

## `PaymentController` (`app/Http/Controllers/PaymentController.php`) — extended Webpay coverage

- **Mutant IDs (user report):** **`transbank`** **`L34–L36`** (`ConcatRemoveLeft`, **`RemoveStringCast`**); **`transbankResponse`** **`L57`** **`EqualToIdentical`**; decline **`$responseMessage`** / **`switch`** **`L70–L108`** (`EmptyStringToNotEmpty`, **`BreakToContinue`**, **`DecrementInteger`/`IncrementInteger`** on case labels — many **UNCOVERED** in Pest because **`continue` inside `switch` behaves like `break`** in PHP, and case-label mutants are hard to observe distinctly).
  - **Cause:** Tests asserted **`endsWith`** on callback URLs but not exact path segments; **`initTransaction`** string casts were not type-pinned; success branch used integer **`0`** only; decline mapping was a long **`switch`** whose label mutants duplicated messages (**`-7` / `-8`**) or were MSI-**UNCOVERED**.
  - **Fix:** **`tests/Feature/Http/PaymentControllerWebTest.php`** — **`parse_url`** host/scheme/path pins on **`lastInit`**; string **`buyOrder`/`sessionId`**; **`responseCode === '0'`** success; **`test_transbank_response_maps_negative_decline_codes_minus_three_through_minus_eight`**; unknown code **`error:999:`**. **`PaymentController`**: replaced the decline **`switch`** with an explicit **`$declineMessages[$code]`** map (and **`-8` → `Rubro no autorizado.`** per inline Transbank legend) so **`RemoveArrayItem`** on message keys is test-detected; **`payment_status`** still uses the raw **`$output->responseCode`** in the outer segment for stable logging.

*(Job mutant notes for **`SendPasswordResetEmail`** / **`SendDeleteAccountRequestEmail`** live earlier in this file under those headings — consolidated with user-report line:operator IDs.)*
...
- **Cause:** **`RemoveEarlyReturn`** after session-based update (**`9876e42b2733194d`**); **`EqualToIdentical` / `NotEqualToNotIdentical`** on **`user_id`** comparisons (**`48a51a12f9e9292f`**, **`a33399cec7d3a7c1`**, **`7524eaeff5a1d671`**, **`e06326715f4fb45e`**); **`RemoveMethodCall`** on **`unregisterDevice`** (**`19189ea765d421e9`**); **`AlwaysReturnNull`** on **`logoutDevice`** (**`9f0dd4874b7b758e`**); **`RemoveMethodCall` / `RemoveArrayItem`** on **`\\Log::info`** / **`\\Log::error`** in **`logoutAllDevices`** (**`d843c0f74e121f8a`**–**`6481e59074926a3f`**, **`0ac4d3da2a8307e2`**–**`d086a6aeac30c9fb`**, **`63bae22d0e6b2c7b`**, **`c8ef9e10e054e933`**); **`RemoveMethodCall`** on completion log (**`0028aa63b2be74fd`**–**`b84ec2922c4b2a6e`**). **`logoutAllDevices`** **`catch`** had no runtime entry while **`FirebaseService::unregisterDevice`** was a no-op stub, so MSI reported **`UNCOVERED`** on **`\\Log::error`** / context keys / **`delete`** / **`$count++`** (**`7477f0453ed06c39`**, **`e4009e822c150e2e`**, **`69f7e8caf08b4d46`**, **`6469c54fd15c2847`**, **`f5ba4072152a1fb5`**, **`d7b64ccb1e27ef1e`**).
- **Fix:** **`DeviceManagerTest::test_register_updates_existing_row_when_session_id_matches`** now asserts **`$second instanceof Device`** so a removed early return cannot pass as **`null`**; **`test_cleanup_inactive_devices_respects_custom_day_threshold`** freezes **`Carbon::now()`**, sets **`last_activity`** older than a custom **`$day`** threshold, and asserts deletion only when that threshold is honored (MSI-style **`DecrementInteger` / `IncrementInteger`** / empty-loop mutants on the cleanup sweep); **`test_register_same_session_keeps_single_database_row`** registers twice with the same **`session_id`** and asserts a single persisted row (**`RemoveEarlyReturn`** on the session dedup path). **`test_logout_device_returns_false_when_session_belongs_to_another_user`** pins **`logoutDevice`** **`false`** vs **`true`** and keeps the row when **`user_id == $user->id`** is required; **`test_logout_all_devices_emits_summary_log_line`** (**`Event::fake(MessageLogged::class)`**) requires the **`Logout all devices completed`** line with **`user_id`** and **`devices_removed`** context so log mutants and stray **`$count--`** fail. **`DeviceManager`** accepts an optional **`FirebaseService`** (default **`new FirebaseService()`**) so tests can inject a subclass whose **`unregisterDevice`** throws; **`test_logout_all_devices_when_fcm_unregister_throws_still_deletes_and_logs_error`** asserts **`devices_removed === 1`**, DB row gone, and a **`MessageLogged`** **`error`** with **`device_id`**, **`device_token`**, and **`error`** message text.

## `PassengersManager` (`app/Services/Logic/PassengersManager.php`)

- **Cause:** **`RemoveArrayItem`** on **`user_id`** validation rule (**`8c713bfafe4fa44a`**); **`GreaterToGreaterOrEqual`**, **`GreaterToSmallerOrEqual`**, **`DecrementInteger`**, **`IncrementInteger`**, **`IfNegated`**, **`RemoveArrayItem`**, **`RemoveMethodCall`**, **`RemoveEarlyReturn`** on **`module_user_request_limited`** block (**`3e4034e8f949e3c5`**–**`a3d1412e2efd7357`**); **`IfNegated`**, **`RemoveFunctionCall`** on auto-accept payment branch (**`8b60197dfc15f8d7`**–**`bab68916cea5798d`**); **`EqualToIdentical`**, **`IfNegated`** on cancel/accept paths (**`31a5e3b2a104fcd8`**, **`00b111038c4a97ca`**); **`IfNegated`**, **`EqualToNotEqual`**, **`EqualToIdentical`** on **`cancelRequest`** **`$trip->user_id == $user->id`** (**`8ffc2c38d2934575`**–**`7c60a81065f9650c`**); **`GreaterOrEqualToGreater`**, **`RemoveMethodCall`**, **`Concat*`** on **`sendFullTripMessage`** (**`059139180cf4a6d0`**–**`ecd546998066a237`**); **`RemoveEarlyReturn`**, **`IfNegated`**, **`RemoveFunctionCall`**, **`AlwaysReturnNull`** on **`acceptRequest` / `payRequest`** (**`2cb749dfd6bce484`**, **`a0d3c2f6639a11f6`**, **`f6aaeecc00f31baa`**, **`7accde37c569c3b0`**, **`1307392731297ed4`**); **`RemoveEarlyReturn`**, **`BooleanOrToBooleanAnd`**, **`RemoveArrayItem`**, **`RemoveMethodCall`**, **`RemoveEarlyReturn`** on **`rejectRequest`** (**`9d7deb1a885c3b36`**, **`70e736f65a41dc2f`**, **`e10aade1829c3a66`**–**`00e5062d174c9586`**). **`rejectRequest`** **`not_valid_request`** branch (**`7150bc5043a39164`**, **`8820f6f02012d44a`**, **`76e57cd06df12f2f`**): no test previously drove **`!tripOwner`** with a still-**`PENDING`** row, so mutants on **`setErrors`** / **`return`** were **`UNCOVERED`**.
- **Fix:** **`test_internal_request_validation_requires_user_id`** (reflection on private **`validateInput`**); **`test_new_request_blocks_when_user_request_limited_module_detects_similar_trip`** seeds an accepted passenger on another trip inside the configured **`hours_range`** window; **`test_cancel_request_passenger_while_waiting_payment_sets_while_paying_state`** asserts **`CANCELED_PASSENGER_WHILE_PAYING`**; **`test_accept_request_moves_to_waiting_payment_when_seats_payment_module_enabled`** + **`test_new_request_autoaccept_with_payment_module_dispatches_auto_and_accept_events`** cover the payment-module branches without over-mocking the repository; **`test_pay_request_returns_null_when_passenger_not_waiting_payment`**, **`test_pay_request_returns_early_when_input_invalid`**, **`test_reject_request_returns_early_when_input_invalid`**, **`test_accept_request_returns_early_when_input_invalid`** pin early returns and truthy **`payRequest`** success (**`test_pay_request_completes_waiting_payment_and_dispatches_reject_event`** extended with **`assertInstanceOf` / `assertTrue`**). **`test_cancel_request_passenger_pending_self_cancel`** passes **`(string) $passenger->id`** as **`$cancelUserId`** so production **`(int) $cancelUserId === (int) $user->id`** cannot be weakened to loose **`==`** without breaking self-cancel. **`test_send_full_trip_message_skips_when_accepted_count_is_below_total_seats`** loads a trip with more **`$trip->seats_available`** than accepted passengers and expects **`ConversationsManager::sendFullTripMessage`** **never**, pinning **`count($trip->passengerAccepted) >= $trip->seats_available`** (not **`total_seats`** alone). **`rejectRequest`** guard (**`~317–320`**): **`RemoveArrayItem`** / **`RemoveMethodCall`** on **`setErrors(['error' => 'not_valid_request'])`** and **`RemoveEarlyReturn`** (**`7150bc5043a39164`**, **`8820f6f02012d44a`**, **`76e57cd06df12f2f`**) stayed **`UNCOVERED`** until **`test_reject_request_non_owner_cannot_reject_pending_passenger`** — a non-owner must not reach **`PassengersRepository::rejectRequest`** (the repo does not re-check ownership, so dropping the **`return`** would wrongly reject the row and dispatch **`RejectEvent`**).
...
- **Fix:** Existing **`NotificationManagerTest::test_get_notifications_paginates_when_page_and_page_size_present`**, **`test_get_notifications_does_not_paginate_when_page_size_is_missing`**, and **`test_get_notifications_does_not_paginate_when_page_is_missing`** already fail if **`&&`** becomes **`||`** (row counts and ordering diverge). **`test_get_notifications_returns_read_and_unread_rows_when_not_marking`** continues to mix read/unread **`readed`** flags. **`test_get_notifications_readed_is_true_when_database_read_at_is_set`** updates the **`notifications`** row’s **`read_at`** and asserts **`readed` === true** so **`read_at != null`** cannot survive **`NotEqualToNotIdentical`** (must stay **`!== null`**).

## `UsersManager::__construct` (`app/Services/Logic/UsersManager.php`)

- **Cause (MSI report ~43–47):** **`TernaryNegated`**, **`CoalesceRemoveLeft`**, **`FalseToTrue`** on **`$carsRepository ?: new CarsRepository()`** and **`$userEditablePropertiesService ?? new UserEditablePropertiesService()`** — swapping **`?:`** / **`??`** or dropping the default **`new`** leaves **`null`** dependencies that only integration would catch.
- **Fix:** **`UsersManagerTest::test_manager_resolves_default_cars_repository_and_editable_properties_service`** resolves **`UsersManager`** from the container without constructor args and uses reflection to assert **`carsRepository`** is **`CarsRepository`** and **`userEditablePropertiesService`** is **`UserEditablePropertiesService`**.

## `UsersManager` — `bankData`, reCAPTCHA `create`, `update` guardrails, `uploadDoc` / `updatePhoto`, driver admin mail (MSI UNCOVERED, Apr 2026 report)

- **Cause:** **`bankData`** path concat / **`json_decode(..., true)`** / **`(object)`** return mutants (**`1826ef5670b927cf`**–**`1f161b7de5d03cdb`**) had no fixture-backed test. **`create`** reCAPTCHA branch (**`~143–200`**: **`TrueToFalse`** on **`$data['active']`**, **`RemoveMethodCall` / `Concat*`** on logs, **`RemoveArrayItem`** on **`$recaptchaData` / `$options`**, **`file_get_contents` / `json_decode`** flags, **`if ($res['success'] == true && $res['score'] >= 0.5)`**, banned-word loop, **`CreateEvent`**, failure **`return false`**) never ran without Google HTTP. **`update`** validator early exit (**`~231–232`**: **`RemoveMethodCall` / `RemoveEarlyReturn`** on **`setErrors`/`return`**) lacked a password-too-short case after **`filterForUser`**. **`uploadDoc` / `updatePhoto`** (**`~340–391`**) MSI clusters were integration-only. **`update`** driver admin **`\\Mail::send`** block (**`~308–317`**) compared **`is_array($data['driver_data_docs'])`** *after* docs were JSON-encoded for persistence, so the branch was effectively dead and **`Mail::fake()->assertOutgoingCount`** could never observe **`Mail::send(view, …)`** (Mail fake only records **`Mailable`** instances).
- **Fix:** **`fetchRecaptchaVerificationResponse($url, $options)`** wraps **`stream_context_create` + `file_get_contents`**; tests use an anonymous **`UsersManager`** subclass to return deterministic JSON (success / failure) without network. **`test_bank_data_returns_decoded_json_from_storage_banks_and_cc_paths`** writes minimal **`{storage}/banks/{country}.json`** and **`cc/{country}.json`**, asserts decoded **`cc`** / **`banks`**, restores prior files. **`test_create_with_recaptcha_*`** + **`test_create_with_recaptcha_marks_banned_when_name_matches_config_banned_word`** pin **`active`**, captcha failure **`false`**, and banned-word handling. **`test_update_returns_when_validator_fails_and_sets_password_errors`**. **`test_upload_doc_*`** / **`test_update_photo_updates_profile_from_valid_jpeg_data_uri`**. **Production fix:** capture **`$driverDocsNotifyAdmin`** before encoding docs, send mail using **`$mailPayload`** (avoid clobbering **`$data`**), **`Mail::shouldReceive('send')`** expectation in **`test_update_sends_driver_request_email_when_admin_configured_and_docs_present`**.

- **Follow-up (Apr 2026, ~96% → 100% MSI on `UsersManager.php` + `UsersManagerTest.php`):** Remaining survivors were **`file_get_contents(..., false, $context)`** **`FalseToTrue`** on **`use_include_path`** (equivalent for HTTP in practice), **`update`** driver **`uploadDoc`** failure guard (**`$uploaded === false`** vs **`!== false` / `=== true`**), non-driver **`else if (is_array(...) && count(...))`**, **`Mail::send`** closure body (**`$message->to(...)->subject(...)`** not exercised when the test only matched **`Mockery::type('Closure')`**), and **`uploadDoc`** JPEG branch (**`File::makeDirectory(..., true)`**, **`.jpg`** concat, **`File::put`**, early **`return`**). **Production:** use **`@file_get_contents($url, context: $context)`** so Infection no longer mutates a meaningless boolean. **Tests:** **`test_fetch_recaptcha_verification_response_reads_local_file_via_context`** (subclass calls **`parent::fetchRecaptchaVerificationResponse`** on a temp file); **`test_update_returns_null_when_driver_upload_doc_fails_validation`** (**`module_validated_drivers`**, **`ImageUploadValidator`** mock **`valid => false`**, assert **`null`** and unchanged **`driver_data_docs`**); **`test_update_non_driver_json_encodes_driver_data_docs_string_paths`**; driver mail test invokes the **`Mail::send`** callback with a **`MailMessage`** mock (**`to`** + **`subject`**); **`test_upload_doc_writes_jpeg_when_converter_returns_non_null_bytes`**; **`test_upload_doc_calls_make_directory_recursive_when_docs_folder_missing`** (**`File::shouldReceive('makeDirectory')->with(..., 0755, true)`** + **`put`** delegate to **`file_put_contents`**).
...
- **Cause:** **`RemoveArrayItem`** on **`PASSWORD_RESET_COOLDOWN`** context keys (**`remaining_minutes`**, **`last_reset_at`**, **`timestamp`**, **`user_id`**, **`email`**) — IDs **`2f7195c9b5dbb98e`**, **`5d5a58f52712840e`**, **`d1bd9e839d1cd3f8`**. **`RemoveMethodCall` / `DecrementInteger` / `IncrementInteger`** on **`\\Log::info('resetPassword before queuing email')`**, **`SendPasswordResetEmail::dispatch(…)->delay(now()->addSeconds(10))`**, **`\\Log::info('resetPassword email queued successfully', …)`**, **`PASSWORD_RESET_QUEUED`** payload / condition / token masking — IDs **`c55f5984878945bd`**, **`0911cb72bad861ee`**, **`590970f0fab71d86`**, **`06f9f73f0aa27ddd`**, **`ad20bdee2550314a`**, **`07da8649d906c73b`**, **`23d44dc27f033c22`**, **`42ed43ca10f59aee`**, **`d0bf54075d271e76`**, **`4b4a662cce00a4c0`**, **`1a8e79d2c958b0fc`**, **`3e0d2ac4b07d01ef`**, **`8024a663856bbef8`**, **`abd0a04142a822e7`**, **`0e2b84387abe1a8b`**, **`db37bae9691b3153`**, **`7eaae9889da48e21`**, **`7e95e8e42e434d53`**, **`998ba61f8dcbc57c`**, **`29f286a9782625f2`**. **`RemoveArrayItem` / `RemoveMethodCall` / `IfNegated`** on unknown-email **`\\Log::warning`** and **`PASSWORD_RESET_USER_NOT_FOUND`** — IDs **`f7303274fb9ec815`**, **`dd0e13e02c96950b`**, **`3f2141249745e087`**, **`c2b28a2636292503`**, **`9619577707ffc697`**, **`5475802669a1d5ea`**, **`6f3195f99b1a9c7f`**.
- **Fix:** **`test_reset_password_cooldown_logs_password_reset_cooldown_with_full_email_logs_context`** (ordered **`Log`/`email_logs`** expectations including **`remaining_minutes`**, **`last_reset_at`**, **`timestamp`**, and the human cooldown line). **`test_reset_password_with_email_logging_queues_job_on_emails_with_ten_second_delay_and_logs_queued_payload`** pins **`emails`** queue, **`now()->addSeconds(10)`** delay via frozen **`Carbon`**, **`resetPassword before queuing email`**, **`resetPassword email queued successfully`** with **`user_id` + `email`**, and **`PASSWORD_RESET_QUEUED`** token shape (**`substr(..., 0, 10) . '...'`**). **`test_reset_password_unknown_user_logs_warning_and_email_logs_when_enabled`** pins default-channel warning payload and **`email_logs`** **`PASSWORD_RESET_USER_NOT_FOUND`** including **`email`**, **`timestamp`**, and **`ip`** with a synthetic **`Request`**.

- **Cause:** **`FalseToTrue`** on **`validator(…, false, false)`** boolean flags (**`20becc52fc0a0e39`**, **`22476444cf33d413`**); **`TrueToFalse`** on **`$data['active'] = true`** (**`48572c5f3ed25b14`**); **`RemoveMethodCall`** on **`deleteResetToken`** (**`094df135c026ba68`**).
- **Fix:** **`test_change_password_activates_inactive_user_and_rejects_reuse_of_consumed_token`** asserts **`active`** becomes **`true`** after a successful reset and that a second call with the same token is rejected (token must be cleared); together with existing validator/password tests this blocks **`active = false`** and missing **`deleteResetToken`** regressions.

- **Cause:** **`EqualToIdentical`** on **`$type == Passenger::TYPE_CONDUCTOR`** / **`TYPE_PASAJERO`** in **`tripsCount` / `tripsDistance`** (**`83a506a0a5a64532`**, **`246b05c176e502f1`**).
- **Fix:** **`test_trips_count_and_distance_treat_string_zero_and_one_like_integer_passenger_types`** passes **`(string)`** **`0`** / **`1`** and asserts the same finished-trip counts and distances as the integer constants.

- **Cause:** **`RemoveMethodCall` / `Concat*`** on **`migrateUsers`** **`\\Log::info('Test COMMAND exit' . $exitCode)`** (**`37d69668c1bbddcb`**, **`c2793c067ae3008e`**, **`31d79e0f97848bdf`**, **`c9fa353b2c277a2d`**). **`Artisan::call('test:test')`** had no registered command in the test harness.
- **Fix:** **`routes/console.php`** registers a no-op **`test:test`** command when **`app()->runningUnitTests()`** is true. **`test_migrate_users_invokes_test_artisan_command_and_logs_exit_code`** asserts **`Artisan::call('test:test') === 0`** and that **`migrateUsers`** logs **`Test COMMAND exit0`**.

- **Cause:** **`ConcatRemove*` / `ConcatSwitchSides`** on **`termsText`** language path (**`6e1784d659f64fd3`**, **`7ccf8836919d3adf`**, **`2663077cddc11a55`**, **`2b38f2851aa4f9b9`**, **`dddc6b5c72449ee2`**, **`0212326a3e2aec84`**, **`0e5d1d99332e40d5`**, **`582c7db04dd7b0bb`**, **`1073f51c9ce45e9a`**, **`3fe587fdd83a1105`**, **`58b72859522fbce4`**, **`e2758ab7243443de`**).
- **Fix:** **`test_terms_text_reads_language_specific_path_then_default_when_lang_empty`** writes **`{storage}/terms/{target_app}_es.html`** and **`{target_app}.html`** with distinct contents and asserts **`termsText('es')`** vs **`termsText('')`** return the expected HTML (cleanup in **`finally`**).

## `SmsService` (`app/Services/SmsService.php`)

- **Cause:** **`RemoveMethodCall` / `Concat*`** on outer **`catch`** **`Log::error('SMS sending failed: …')`** (**`aa6072ded6136949`**, **`b37ed42936b9b0a9`**, **`8b5f475b8519bc58`**, **`58e7c99f309d9efe`**); **`FalseToTrue`** on **`return false`** (**`cf8e5ce992d4508e`**). **`CoalesceRemoveLeft` / `IfNegated` / `BooleanOrToBooleanAnd` / `RemoveNot` / `RemoveMethodCall` / `FalseToTrue` / `RemoveEarlyReturn`** on WhatsApp config guard (**`96feadc30155c576`**–**`bec8816a46508491`** and related). Local Graph **`Http::post`** payload / options / success predicates — large cluster (**`0b65f7a8caa13247`** through **`14e2d69f8a3f30ef`** in the user report).
- **Fix:** **`test_send_logs_exception_message_and_returns_false_when_inner_send_throws`** uses a tiny anonymous **`SmsService`** subclass that throws from **`sendViaWhatsApp`** and asserts the outer catch logs **`SMS sending failed: …`** and returns **`false`**. **`test_send_whatsapp_returns_false_when_configuration_is_incomplete`** asserts **`WhatsApp configuration missing`**. **`test_send_whatsapp_uses_local_http_branch_with_full_graph_template_payload`** sets **`$app['env'] = 'local'`**, **`Http::fake`**, pins URL (**`default_graph_version` + `phone_number_id`**), **`Authorization: Bearer …`**, **`verify => false`**, template name/language, six-digit code extraction, and **`expires`** minutes from **`config('sms.verification.expires_in_minutes')`**. **`test_send_whatsapp_local_http_returns_false_when_graph_response_has_no_wa_message_id`** pins the **`successful()`** + **`messages[0]['id']`** gate.

- **Cause:** Local WhatsApp **`isset($body['messages'][0]['id'])`** cluster (**`DecrementInteger` / `IncrementInteger` / `IfNegated`** on index **`0`** vs **`-1` / `1`**, e.g. **`4c8ff37fbfd142c1`**, **`19e0e1ddb079d786`**, **`2c7fbd4d2ba42f7e`**, **`2b2e4db3b504dcb7`**); success-path **`Log::info`** / **`return true`** / error **`Log::error`** mutants on **`~121–128`** (**`2445023bc4ea5137`**, **`7b25f1f0159f5d8b`**, **`6bf5c075ff83749c`**, **`f0209b11859b2ff3`**, **`6de7a90e0d3ebcbf`**, **`89d7a7c799c2f11e`**, **`b6979397fa542d7c`**, **`1f68aa663f54a0b8`**, plus adjacent **`Concat*`** IDs in the user report); non-successful HTTP branch (**`b6979397fa542d7c`**, **`1f68aa663f54a0b8`**, **`Concat*`** on **`131`**).
- **Fix:** **`test_send_whatsapp_local_http_requires_wa_message_id_on_index_zero_not_later_entries`** fakes Graph **`200`** with **`messages[1]['id']`** only — must stay **`false`** and log **`WhatsApp API error via Laravel HTTP client:`** (also pins the prior **`Log::info('WhatsApp API URL being called', …)`** so Mockery does not swallow **`Http::post`** inside the inner **`catch`**). **`test_send_whatsapp_local_http_logs_transport_failure_including_status_and_body`** expects **`WhatsApp HTTP client failed with status: 429 - rate limited`**. **`test_send_whatsapp_local_http_logs_success_line_including_formatted_phone_and_message`** orders **`Log::info`** for the URL line then the exact **`via Laravel HTTP client to: 541122223333 with message: …`** string.

- **Cause:** Inner **`catch`** **`Log::error('Laravel HTTP client failed: …')`** and Facebook fallback / **`Log::info('WhatsApp API request details', …)`** / **`$fb->post`** / second **`messages[0]['id']`** / outer **`WhatsApp sending failed`** / **`extractCodeFromMessage` / `formatPhoneForWhatsApp` / `sendViaSmsMasivos` / `formatPhoneForSmsMasivos`** — large PEST lists (**`0e81eac677d36c40`** through **`7114540942288584`** for **`SmsService.php`**, including **`sendViaSmsMasivos`** **`~249–335`**).
- **Fix:** **`sendViaSmsMasivos`** / **`formatPhoneForSmsMasivos`**: **`test_send_sms_masivos_*`** (**`568e2a13bf5d3d3c`**, **`99c1cb795625e398`**, **`223e76bcd1035082`**, **`b664278c6b0097f8`**, **`263`–`303`** **`RemoveArrayItem` / `IfNegated` / `DecrementInteger`**, **`278–303`** branch mutants) via **`Http::fake`** on **`servicio.smsmasivos.com.ar`**, exact log strings for **`OK`**, **`probando sin enviar`**, **`SMS Masivos error:`**, **`SMS Masivos HTTP error:`**, and **`Http::assertSent`** proving **`test => 1`** when **`test_mode`** is true. **`test_sms_service_harness_extracts_six_digit_code_from_message`** / **`test_sms_service_harness_falls_back_to_generate_when_no_six_digit_token`** pin **`preg_match` / `matches[1]`** vs fallback (**`c6094dafa9361349`**, **`7013acbe70ea45e4`**, **`fcbe0139d9f221aa`**, **`03c62f528234ffae`**, **`2150589e346725dd`**). **`test_sms_service_harness_formats_whatsapp_numbers_for_argentina`** and **`test_sms_service_harness_formats_sms_masivos_numbers_and_warns_on_unexpected_length`** pin **`formatPhoneForWhatsApp`** / **`formatPhoneForSmsMasivos`** guards and the **`strlen($phone) !== 10`** warning (**`461a3d418c9bcb22`**–**`7114540942288584`** style mutants). The **Facebook SDK** branch after a successful local **`Http`** response is not executed in these tests (no **`new Facebook`** on the happy/error paths above); surviving mutants there still need either the **`facebook/graph-sdk`** dependency plus an **`HttpClient`/`Facebook` test double**, or a future seam — not covered here.

## `FirebaseService` (`app/Services/FirebaseService.php`)

- **Cause:** **`EmptyStringToNotEmpty`** on default properties; **`AlwaysReturnNull`** on **`getAccessToken`**; **`Concat*`** on FCM URL; **`RemoveArrayItem`** / **`UnwrapStrtolower`** / **`IfNegated`** / **`ForeachEmptyIterable`** / **`BooleanOrToBooleanAnd`** / **`RemoveStringCast`** on **`switch ($deviceType)`** branches; **`RemoveArrayItem`** / **`BreakToContinue`** on **`android` / `apns` / `webpush`** payloads; **`json_decode(..., true)`** vs **`false`**; **`ClientException`** / **`RequestException`** logging (**`32ba1116b2a67027`** through **`3addafd75a9a5670`** in the user report).
- **Fix:** Production adds **`protected function fetchMessagingAccessToken(): array`** and **`protected function httpClient(): HttpClient`** so tests can substitute Guzzle without **`overload:`**. **`FirebaseServiceHarness`** (same test file) sets config, overrides **`fetchMessagingAccessToken`** + **`httpClient`**, and uses **`Reflection`** to align **`firebaseName`**. **`FirebaseServiceTest`** asserts **`getAccessToken`** reads **`['access_token']`**, **`sendNotification`** for **`android` / `IOS` / `kiosk`** builds the expected FCM **`v1/projects/{project}/messages:send`** URL, **`Authorization: Bearer …`**, **`Content-Type`**, and per-platform JSON (**`android.notification` + `data` stringification**, **`apns.payload.aps` + `headers` + parallel `data`**, **`webpush`** default branch). **`ClientException`** / **`RequestException`** cases assert **`\Log::error`** prefixes and key context fields, then expect rethrow.
...
- **Cause (Apr 2026 Infection report):** **`AlwaysReturnNull`** on **`getUserData`** / **`getError`** (**`d655de58a23d3fa2`**, **`2f868f6e60334bd9`**); **`RemoveArrayItem`**, **`TernaryNegated`**, **`FalseToTrue`** on the profile array returned from **`getUserData`** (**`7b2e440e96b05e57`**, **`1c5ab6b5bdfba114`**, **`eeea3f5ce15e1367`**, **`91abd4d82e8e8abb`**, **`0e435085b61330a8`**, **`179a735ff5dc62cd`**, **`4d488212c81ea890`**, **`ab8a5fa870f76ee4`**, **`595c0e8f5672d3eb`**, **`0cb1cbb9d2a47a3a`**, **`04f741353f7a7361`**).
- **Fix:** same test file asserts the full associative shape (**`provider_user_id`**, **`email`** as **`null`** when absent, **`name`**, **`gender`**, **`birthday`**, **`banned`**, **`terms_and_conditions`**, **`image`**) and uses reflection to set **`$error`** then assert **`getError()`** returns that payload (not a hard-coded **`null`**).
...
- **Cause:** outer **`send()`** catch (**`PushChannel: Error sending push notification`**) **`\\Log::error`** context was only weakly asserted, so **`CoalesceRemoveLeft`**, **`RemoveArrayItem`**, **`substr` / concat / integer nudge**, and **`UnwrapSubstr`** mutants on **`user_id`**, **`device_id`**, **`device_token`**, **`device_type`**, **`error`**, **`error_trace`** survived (e.g. **`0f98ba076425f67a`**, **`394b2f7565d26b1a`**, **`e9de4475505b3155`**, **`f3cee64910efcb24`**, **`0d58121374c85b88`**, **`e3187e575924eb58`**, **`6b8a48ed1ac78707`**, **`c21f1e9d312a7003`**, **`5e89a170eace3568`**, **`d56317a30ddc5c8a`**, **`719dd0037bacc2c7`**, **`f2bfb91492403d41`**, **`89509ac4a7b1e290`**, **`01def22738581cc9`**, **`78eb2d51f6152169`**, **`45b7e89c8d95080a`**, **`84e9c2b1a0ad53c7`**, **`3854d24d33414a31`**).
- **Fix:** same test file records all **`Log::error`** invocations (Firebase noise tolerant) and asserts the outer log’s **`user_id`**, persisted **`device_id`**, redacted **`device_token`** (**`substr(..., 0, 20) . '...'`**), **`device_type`**, non-empty **`error`**, and substantive **`error_trace`**.

- **Cause:** **`sendAndroid`** inner catch (**`PushChannel: sendAndroid error`**) array keys and redaction mirrored the outer cluster; **`RemoveMethodCall`** on **`\\Log::error`** and related mutants were likewise uncovered (e.g. **`5801e8d516c48e71`**, **`1b088d4bdafbf20b`**, **`519ceb93e10fc9bb`**, **`3cdb433bad070be7`**, **`5ef871f5d55f165f`**, **`21020ed268f7a722`**, **`290a548b039a67d9`**, **`59d1f3ff2426b3b1`**, **`16194f1c56c1a9af`**, **`fb3cc2c08a706b5c`**, **`65309dd946309d50`**, **`37e5cc02c8ebf10d`**, **`6ae2a25a2cce6cc0`**, **`e529bfc1fcf404b4`**, **`ba3ae3ce8012aea4`**, **`ae72ea73617f8ecb`**, **`4fd18a13c1474edc`**, **`229639e1e841fac8`**).
- **Fix:** assert a matching inner log with **`device_id`**, **`device_token`**, **`error`**, **`error_trace`**, and **`input_data`** containing the notification’s **`message`** / **`title`** (proves logging + payload forwarding before **`throw $e`**).
...
## Console commands — Infection (`app/Console/Commands/*`, Apr 2026)

### `FacebookImage` (`app/Console/Commands/FacebookImage.php`)

- **Cause:** **`RemoveMethodCall`** on **`\\Log::info('COMMAND FacebookImage')`** (**`9b1b20429b311d99`**); **`RemoveMethodCall`** / empty closure on **`whereHas('trips', … trip_date >= now)`** (**`c2cbea3b45ce26e9`**); **`ForeachEmptyIterable`** / branch mutants on the user loop and **`if ($user->accounts && $user->accounts[0]->provider_user_id)`** (**`63e9db27115a2827`**, **`26fa1b33ed806445`**, **`a126afe8e9a8cf76`**, **`9e18ae8aa38a2c2a`**, **`92ae2e76f141f62b`**, etc.); **`RemoveMethodCall`** / **`Concat*`** on the **`$this->info($user->id . ' ' . $user->name . ' ' . $url)`** line (**`0f1cdfa7b60567f1`**, **`8f4737461abf12cd`**, …); **`RemoveMethodCall`** on **`downloadAndSave` / `save` / else branch** (**`873860904427701e`**, **`3873041d5bb6edcc`**, **`fd4ac36a2680574c`**); **`IfNegated` / `Equal*` / integer nudges** on **`getStatusCode() == 200`** (**`8b53593c880358a9`**, **`ed0b24945e3a5a87`**, **`fa10d5e2496af07b`**, **`97051d95dde641cf`**, **`78bf51dbf8fd7780`**); **`Concat*`** / **`AlwaysReturnNull`** on **`request()`** URL and return (**`5f908f8bce22d1f6`**, …, **`2c98543c6416a854`**); **`ValueError`** / silent bad downloads when Graph returns no usable URL (body stream / empty URL) so **`file_get_contents`** was still invoked.
- **Fix:** optional injectable **`GuzzleHttp\\Client`** in the command constructor; **`requestImages`** reads the PSR-7 body via **`rewind()` + `getContents()`** and **`JSON_THROW_ON_ERROR`**; **`handle()`** skips **`downloadAndSave`** when **`empty($url)`**; **`tests/Unit/Console/Commands/FacebookImageTest.php`** asserts **`MessageLogged`** for **`COMMAND FacebookImage`**, zero vs matched user counts, **`Client::request`** receives the full Graph picture URL, **`FileRepository::createFromData`** + **`$user->image`** on **`200`**, no HTTP call when the only trip is in the past, **`No account`** when **`provider_user_id`** is empty, **`429`** skips persistence (image unchanged) and skips **`createFromData`**; full user line is asserted via **`Symfony\\Component\\Console\\Tester\\CommandTester`** (avoids Laravel’s per-chunk **`expectsOutputToContain`** limits on very long URLs).

### `RatesAvailability` (`app/Console/Commands/RatesAvailability.php`)

- **Cause:** **`RemoveMethodCall`** on **`\\Log::info('COMMAND RatesAvailability')`** (**`aa95f328bc5c911b`**); remaining mutants on the first **`Rating::where…->update`** and join/update chain were already covered by DB assertions in **`RatesAvailabilityTest`**, but the log line was not observed (Laravel 12 has no **`Log::fake()`** on the default logger).
- **Fix:** **`tests/Feature/Commands/RatesAvailabilityTest.php`** — **`test_logs_command_identifier_when_rating_availability_runs`** uses **`Event::fake([MessageLogged::class])`** and asserts a dispatched **`MessageLogged`** with message **`COMMAND RatesAvailability`**.

### `SendAnnouncement` (`app/Console/Commands/SendAnnouncement.php`)

- **Cause:** **`RemoveMethodCall`** / **`IfNegated`** / **`TernaryNegated`** / **`Concat*`** on the configuration banner, statistics block, dry-run / rate-limit / results sections, and **`$this->error`** on failure (**`1529362a32097de5`**, **`cdabeb4e19c090c4`**, **`c2e2df51f5c9d263`**, **`b74cb472b902c10d`**, **`3e6d631e04b8710c`**, **`cc57a717ad1f760e`**, **`aa3be79e9d96d18c`**, **`c13247f6394486a2`**, **`833aa15cf65011fd`**, **`4c222d637a538669`**, **`d880fdd9e0bdb78b`**, **`11dfc41925c3c459`**, **`70616a3b51b67543`**, **`d8c8de7647dde097`**, **`1d28d50c98a0315b`**, **`0eb5947a82d0212b`**, **`05d6cea15f764512`**, **`71fcec9f56aefde7`**, **`f84befd33b750564`**, **`86e48d40fe4c71c7`**, **`f17f2e6ee1535e4d`**, **`1d350e2d75d36ed1`**, **`59e6da992493b3ee`**, **`9619e41ae8b77bff`**, **`3343ea429d8fbb6e`**, **`25438ef2308f9403`**, **`8a319f408afa21b4`**, **`6b6f133687fe3e42`**, **`5b5bc9f4fda75fdc`**, **`b28a897094e89ab2`**, **`ee5a2fc9fc28f84b`**, **`bb0e46046f694baf`**, **`6ef15c15accd332a`**, **`f371ade03b64faab`**, **`2edcd762a195c207`**, **`d29943b2f8c4d843`**, **`23bc69bfd4e7f9c6`**, **`8c4ae29f2afd30b4`**, **`c0238721caf1cfc5`**, **`f3e2bd41d398d230`**, **`bb112e4c93647458`**, **`dedbb7a69815b9d2`**, **`d1dc9f65620892b6`**, **`e0229376d8fa35ac`**, **`7fe5e0ac45b4574d`**, **`e48a380d6b12b3d6`**).
- **Fix:** **`tests/Feature/Commands/SendAnnouncementCommandTest.php`** mocks **`AnnouncementService`** only (no push I/O), drives **`expectsConfirmation`**, and asserts exact **`$this->info` / `$this->warn` / `$this->error`** substrings for banner options, stats, dry-run vs send, rate limit, success stats, failure message, and **`sendToUsers`** vs **`sendAnnouncement`**.

### `CleanupPasswordResetTokens` (`app/Console/Commands/CleanupPasswordResetTokens.php`)
...
- **`Line 28–63: DecrementInteger` / `IncrementInteger`** (numeric **class constants**)
