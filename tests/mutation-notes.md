...
- **Mutant IDs (user report, line:operator):** **`L46`** / **`L120`** **`FalseToTrue`** on **`config('carpoolear.log_emails', false)`**; **`L62`** **`UnwrapSubstr`**, **`IncrementInteger`/`DecrementInteger`** on **`substr`** bounds, **`Concat*`** on token ellipsis; **`L77–L79`** / **`L92–L97`** **`RemoveArrayItem`** on success / catch log payloads; **`L105–L106`** **`UnwrapArrayMerge` / `RemoveMethodCall` / `RemoveArrayItem`** on **`PASSWORD_RESET_EMAIL_FAILED`**; **`L123–L127`** **`RemoveArrayItem`** on **`failed()`** **`$failureData`** / critical merge. **`L19–L21`** (`$tries`, **`$backoff`**, **`$timeout`**) remain **UNCOVERED** in scoped MSI: mutants alter property initializers without **`handle()`** reading them in the same compilation unit, so the suite does not observe changed queue metadata.
  - **Cause:** **`log_emails`** default and optional branches were not fully pinned; token preview allowed loose **`str_starts_with`**; catch / **`failed()`** paths did not assert every **`errorData`/`failureData`** key or **`array_merge`** **`stack_trace`** on **`email_logs`**.
  - **Fix:** **`tests/Unit/Jobs/SendPasswordResetEmailTest.php`** — **`test_handle_when_log_emails_key_is_absent_does_not_use_email_logs_channel`**; **`test_handle_logs_to_email_logs_when_enabled`** now asserts exact **`substr($token,0,10).'...'`** (**length 13**) and **`PASSWORD_RESET_EMAIL_SUCCESS`** keys; **`test_handle_when_mail_fails_and_email_logging_merges_stack_trace_on_channel`**; stricter **`test_handle_rethrows_after_logging_when_mail_fails`** (**`error_code`**, **`attempt`**); **`test_failed_when_log_emails_key_is_absent_does_not_use_email_logs_channel`**; **`test_failed_logs_permanent_failure_and_optional_email_channel`** extended with **`attempts`** / **`timestamp`** on both **`Log::error`** and **`critical`** payloads.

## `SendDeleteAccountRequestEmail` (`app/Jobs/SendDeleteAccountRequestEmail.php`)

- **Mutant IDs (user report, line:operator):** **`L104`** **`FalseToTrue`** on **`failed()`** **`config(..., false)`**; **`failed()`** / **`critical`** **`RemoveArrayItem`** on **`$failureData`** merge. **`L18–L20`** queue property mutants **UNCOVERED** (same reason as password-reset job).
  - **Cause:** **`failed()`** with missing **`log_emails`** config key was not asserted to skip **`email_logs`**; critical merge did not require **`attempts`** alongside **`stack_trace`**.
  - **Fix:** **`tests/Unit/Jobs/SendDeleteAccountRequestEmailTest.php`** — **`test_failed_when_log_emails_key_is_absent_does_not_use_email_logs_channel`**; **`test_failed_logs_permanent_failure_and_optional_email_channel`** tightened **`Mockery::on`** for **`attempts`**, **`timestamp`**, and **`stack_trace`** on **`DELETE_ACCOUNT_REQUEST_EMAIL_PERMANENTLY_FAILED`**.
