...
RateLimiter::for('support-ticket-create', function (Request $request) {
    $user = $request->user();
    if (! $user) {
        return Limit::perHour(config('carpoolear.support_ticket_rate_limit_create_per_hour', 5))->by($request->ip().':support-create');
    }

    return Limit::perHour(config('carpoolear.support_ticket_rate_limit_create_per_hour', 5))->by($user->id.':support-create');
});

RateLimiter::for('support-ticket-reply', function (Request $request) {
    $user = $request->user();
    if (! $user) {
        return Limit::perHour(config('carpoolear.support_ticket_rate_limit_reply_per_hour', 20))->by($request->ip().':support-reply');
    }

    return Limit::perHour(config('carpoolear.support_ticket_rate_limit_reply_per_hour', 20))->by($user->id.':support-reply');
});

RateLimiter::for('support-ticket-admin-reply', function (Request $request) {
    $user = $request->user();
    if (! $user) {
        return Limit::perHour(config('carpoolear.support_ticket_rate_limit_admin_reply_per_hour', 120))->by($request->ip().':support-admin-reply');
    }
