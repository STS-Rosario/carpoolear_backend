...
_ga.getSocialActionTrackers_ = function(network, socialAction, opt_target, opt_pagePath) {
  if ( opt_target ) { 
     opt_target = _ga.extractUriFromURL_(opt_target);
  }
  return function() {
    ...
    if (FB && FB.Event && FB.Event.subscribe) {
      ...
      FB.Event.subscribe('comment.create', function(obj) {
	   ...
	   opt_target = obj.href;
        ...
        _gaq.push(_ga.getSocialActionTrackers_('facebook', 'comment', opt_target, opt_pagePath));
      });
      FB.Event.subscribe('comment.remove', function(obj) {
	   ...
	   opt_target = obj.href;
        ...
        _gaq.push(_ga.getSocialActionTrackers_('facebook', 'uncomment', opt_target, opt_pagePath));
      });
    }
  } catch (e) {}
};
...
_ga.trackTwitterHandler_ = function(intent_event, opt_pagePath) {
  ...
  if (intent_event && intent_event.type == 'tweet' ||
    ...
    if (intent_event.target.nodeName == 'IFRAME') {
      opt_target = _ga.extractParamFromUri_(intent_event.target.src, 'counturl');
 ...
 *
 */
_ga.extractUriFromURL_ = function(url) {
   if ( url ) { 
   try {  
...
/**
*Agrego funcion para traquear el boton de linkedin
*/
