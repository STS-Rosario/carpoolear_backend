...
class TripsManager extends BaseManager implements TripLogic
{
    ...
    public function calcTripPrice ($trip) {
        $country = config('carpoolear.osm_country', '');
        if (config('carpoolear.osm_country', '') == 'ar') {
            return 0;
        } else if ($country == 'cl') {
            ...
            foreach ($ciudades->destinations as $ciudad) {
                if (strpos($trip->from_town, $ciudad->title) !== false) {
                    $slug_origin = $ciudad->slug;
                }
                if (strpos($trip->to_town, $ciudad->title) !== false) {
